 ...
 * asakusa.js
 *
 * 역할: 히어로 우측 2D 캔버스 맵 (台東区 GeoJSON + 건물 파티클)
 */

const BUILDINGS = [
  {
    num:       '01',
    nameEn:    'Asakusa Station Exit 4',
    tag:       'Infrastructure · 1930',
    lat:       35.7109,
    lon:       139.7969,
    intensity: 0.80,
  },
  ...
  {
    num:       '05',
    nameEn:    'Fuglen / Sauna Reset Pint',
    tag:       'Café / Sauna · 2019',
    lat:       35.7131,
    lon:       139.7942,
    intensity: 0.75,
...
const LAT_COS = Math.cos(35.71 * Math.PI / 180);
...
function drawWard(geojson) {
  ...
  /* 3단계: 墨田区(스미다구) — 약간 강조 (강 건너편) */
  const sumida = geojson.features.filter(f => f.properties.name === '墨田区');
  sumida.forEach(f => {
    const polys = f.geometry.type === 'Polygon'
      ? [f.geometry.coordinates]
      : f.geometry.coordinates;
    polys.forEach(poly => {
      ...
      ctx.fill();
      ctx.strokeStyle = 'rgba(26,26,24,0.28)';
      ctx.lineWidth   = 0.8;
      ctx.stroke();
    });
  });

  drawWardLabel('Taito', 139.788, 35.710);
  drawWardLabel('Sumida', 139.804, 35.710);
}
